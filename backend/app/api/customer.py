...
async def delete_order(
    ...
    """
    删除/取消订单：
    - pending/accepted 状态：允许取消（客户/管理员）
    - completed 状态：允许删除（客户/司机/管理员）
    """
    if order_id not in _customer_orders:
        raise HTTPException(status_code=404, detail="订单不存在")
    order = _customer_orders[order_id]
    ...
    # pending/accepted/completed 状态允许删除（不真正删除数据，只确认权限）
    # 司机删除 → 只对司机自己隐藏，客户仍能看到
    # 客户删除 → 只对客户自己隐藏，司机仍能看到
    # 前端各自用 localStorage 黑名单过滤
    if order["status"] in ("pending", "accepted", "completed"):
        return {"status": "ok", "message": f"订单 {order_id} 已标记删除"}

    # in_transit/delivered 不允许删除
    raise HTTPException(
        status_code=400,
        detail=f"订单状态 '{order['status']}' 不允许删除，仅 pending/accepted/completed 可操作",
    )
