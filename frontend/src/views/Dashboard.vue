...
const resourceStats = computed(() => ({
  availableVehicles: store.kpi.total_online_devices || store.devices.filter((d: any) => d.online !== false).length || 0,
  availableWarehouses: store.kpi.warehouse_distribution?.length || 0,
  todayOrders: store.kpi.total_waybills || store.orderFlow.total || 0,
  // 能耗基于在线设备数量和运行小时数估算（每台冷机约28.5kWh/day），不再使用随机数
  energyUsage: Math.round((store.kpi.total_online_devices || store.devices.filter((d: any) => d.online !== false).length || 0) * 28.5),
}))
...
const orderStats = computed(() => {
  ...
  return {
    ...
    matching: flow.accepted || 0,
    inTransit: flow.in_transit || 0,
    completed: (flow.delivered || 0) + (flow.completed || 0),
  }
})
...
const zoneMatch = computed(() => {
  ...
  return {
    freeze: devices.filter((d: any) => d.cargo_zone === 'frozen').length || 0,
    chill: devices.filter((d: any) => d.cargo_zone === 'refrigerated').length || 0,
    ambient: devices.filter((d: any) => d.cargo_zone === 'ambient').length || 0,
  }
})
